...
from attendance_api import AttendanceApiClient
...
def build_argparser():
    ...
    general.add_argument('--reconnect', default=False, action='store_true',
    ...
    general.add_argument('--log_file', default='',
                         help='Optional. Write detailed worker logs to this file.')
    ...
    att.add_argument('--db_url',
                     default=os.getenv('DATABASE_URL', DEFAULT_DATABASE_URL),
                     help='Optional. PostgreSQL attendance DB URL. Set to empty '
                          'string to disable attendance logging.')
    att.add_argument('--api_url', default='',
                     help='Optional. FastAPI base URL. When set, recognition '
                          'events are sent to the API instead of connecting '
                          'directly to PostgreSQL.')
    att.add_argument('--direction', choices=('in', 'out'), default='in',
                     help='Required for two-camera attendance. Use "in" for '
    ...
    att.add_argument('--no_event_snapshots', action='store_true',
                     help='Optional. Do not send face snapshots with recognition '
                          'events. This reduces API latency.')
    att.add_argument('--attendance_cooldown', type=float, default=30.0,
                     help='Optional. Seconds to wait before posting another '
                          'attendance event for the same person from this worker.')
...
class FrameProcessor:
    ...
    def __init__(self, args):
        ...
        if args.api_url:
            self.attendance = AttendanceApiClient(
                args.api_url,
    ...
    def process(self, frame):
        ...
        if self.frame_index % 10 == 1 or len(rois) > 0:
            log.info('STEP detect direction=%s frame=%s faces=%s',
                     self.direction, self.frame_index, len(rois))
        ...
        if self.attendance is not None:
            ...
            for i, ident in enumerate(face_identities):
                if ident.id == SPOOF_ID:
                    ...
                    last_post = self.last_attendance_post.get(name, 0.0)
                    if now_mono - last_post < self.attendance_cooldown:
                        log.info('STEP attendance_skip direction=%s frame=%s name=%s reason=cooldown',
                                 self.direction, self.frame_index, name)
                        continue
                    self.last_attendance_post[name] = now_mono
                    log.info('STEP attendance_post direction=%s frame=%s name=%s confidence=%.3f',
                             self.direction, self.frame_index, name, confidence)
                    marked, time_str = self.attendance.mark(name, confidence, orig_image, rois[i])
                    log.info('STEP attendance_result direction=%s frame=%s name=%s changed=%s time=%s',
                             self.direction, self.frame_index, name, marked, time_str)
                    if marked:
